...
import GeneratePlan from '@/components/plan/GeneratePlan';
...
const Plan = () => {
  ...
  function handleGeneration(months,years){
    setIsLoading(true);
    setPlan(()  =>  generatePlan(months+years*12));
  }
  ...
  useEffect(()=>{
    ...
    return (
      ...
      <div className="flex flex-col min-h-screen overflow-hidden">
        ...
        <GeneratePlan handleGeneration={handleGeneration} setIsLoading={setIsLoading} btn={btn} setBtn={setBtn} />
       {isLoading ?  <div role="status" className=" flex-1   text-center   break-inside-avoid rounded-lg   bg-clip-padding px-12 py-4  backdrop-blur-lg backdrop-filter  w-full h-fit">
        ...
        </div> :  plan.length !== 0 ? <Result duration={duration} plan={plan} /> : null}
